...
import {
  ...
  House, Layers3, Link2, LoaderCircle, Network, Play, Power, RefreshCw, Route,
  Search, Server, Settings2, ShieldCheck, Smartphone, Trash2, Wifi, Zap,
} from 'lucide-react';
...
type LatencyMap = Record<string, LatencyResult | { ok: false; testing: true }>;
...
function compatible(node: EasyServer, core: EasyCore) {
  ...
  if (core === 'xray' || core === 'v2ray') {
    return node.rawKind !== 'sing-box' && String(node.proxy?.type || '').toLowerCase() !== 'hysteria2';
  }
  return !node.rawKind;
}
...
function formatLatency(value?: LatencyResult | { ok: false; testing: true }) {
  if (!value) return '—';
  if ('testing' in value && value.testing) return '…';
  if (value.ok && value.ms != null) return `${value.ms} ms`;
  return value.error || 'нет ответа';
}
...
export function TabEasy({ status, config, actionLoading, handleServiceAction }: Props) {
  ...
  const [network, setNetwork] = useState({
    proxyMode: Number(config.PROXY_MODE ?? 0),
    dns: Number(config.DNS_HIJACK_ENABLE ?? 1) !== 0,
    wifi: Number(config.PROXY_WIFI ?? 1) === 1,
    mobile: Number(config.PROXY_MOBILE ?? 1) === 1,
    udp: Number(config.PROXY_UDP ?? 1) === 1,
    ipv6: Number(config.PROXY_IPV6 ?? 0),
    blockQuic: Number(config.BLOCK_QUIC ?? 0) === 1,
    performance: Number(config.PERFORMANCE_MODE ?? 0) === 1,
  });
  ...
  const activate = useCallback(async (candidate: EasyState, desiredCore = candidate.core) => {
    ...
    try {
      ...
      if (changedBin && previousCore !== desiredCore) {
        try {
          await boxBridge.setConfig('bin_name', previousCore);
  ...
  const importValue = async () => {
    ...
    await run('import', 'Скачиваю и разбираю подписку', async () => {
      ...
      if (/^https?:\/\//i.test(value)) {
        // Important: backend import is staging-only while Easy is disabled.
        next = await timeout(easyBridge.addSubscription(name.trim(), value), 55000, 'Импорт подписки');
      } else {
  ...
  const switchCore = async (core: EasyCore) => {
    ...
    await run(`core:${core}`, `Устанавливаю ${core}`, async () => {
      ...
      let next = await timeout(easyBridge.setCore(core), 150000, 'Установка core');
      if (state.easyEnabled) {
        try {
          await boxBridge.setConfig('bin_name', core);
          await handleServiceAction(status.running ? 'restart' : 'start');
        } catch (e) {
          await easyBridge.setCore(before);
          await boxBridge.setConfig('bin_name', previousBin);
          if (status.running) await boxBridge.service('restart');
          throw e;
        }
      }
  ...
  const selectNode = async (ref: string) => {
    await run('select-node', 'Переключаю узел', async () => {
      const next = await easyBridge.selectNode(ref);
      setState(next);
  ...
  const saveNetwork = async () => {
    await run('network', 'Применяю сетевые настройки', async () => {
  ...
  const stopEasy = async () => {
    await run('disable', 'Возвращаю предыдущую конфигурацию', async () => {
  ...
  return <div className={`relative min-h-full pb-28 ${bg}`}>
    <div className="mx-auto max-w-md px-4 pt-3">
      {view === 'home' && <div className="space-y-4">
        <section className="relative overflow-hidden rounded-[32px] bg-indigo-600 p-6 text-white shadow-lg">
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div>
              <div className="mb-2 flex items-center gap-2 text-xs font-semibold text-indigo-100"><ShieldCheck size={16}/> ROOT · TPROXY</div>
              <h2 className="text-2xl font-black tracking-tight">{status.running ? 'Защита включена' : 'Прокси остановлен'}</h2>
              <p className="mt-2 text-sm text-indigo-100">{state.core} · {nodes.length} узлов · {state.subscriptions.length} подписок</p>
            </div>
            <div className={`flex h-14 w-14 items-center justify-center rounded-full ${status.running ? 'bg-emerald-400 text-emerald-950' : 'bg-white/15'}`}><Power size={26}/></div>
          </div>
          <div className="relative mt-6 grid grid-cols-3 gap-2">
            <button disabled={Boolean(actionLoading)} onClick={() => void handleServiceAction(status.running ? 'stop' : 'start')} className="rounded-2xl bg-white/15 px-2 py-3 text-xs font-bold backdrop-blur">{status.running ? 'Остановить' : 'Запустить'}</button>
            <button onClick={() => void handleServiceAction('restart')} className="rounded-2xl bg-white/15 px-2 py-3 text-xs font-bold backdrop-blur">Рестарт</button>
            <button onClick={() => setView('settings')} className="rounded-2xl bg-white/15 px-2 py-3 text-xs font-bold backdrop-blur">Настройки</button>
          </div>
        </section>

        <section className={`${card} p-5`}>
          <div className="flex items-center gap-3"><div className="rounded-2xl bg-indigo-100 p-3 text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-300"><Link2 size={22}/></div><div><h3 className="font-bold">Добавить подписку</h3><p className="text-xs text-slate-500 dark:text-slate-400">URL, JSON, Base64 или URI</p></div></div>
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Название · необязательно" className="mt-4 w-full rounded-2xl bg-slate-100 px-4 py-3 text-sm outline-none ring-indigo-500 focus:ring-2 dark:bg-white/5"/>
          <textarea value={input} onChange={e => setInput(e.target.value)} rows={4} placeholder={'Вставь ссылку подписки\nили vless://… / vmess://… / hysteria2://…'} className="mt-2 w-full resize-none rounded-2xl bg-slate-100 px-4 py-3 text-sm outline-none ring-indigo-500 focus:ring-2 dark:bg-white/5"/>
          <button disabled={!input.trim() || Boolean(busy)} onClick={() => void importValue()} className="mt-3 flex w-full items-center justify-center gap-2 rounded-[20px] bg-indigo-600 py-3.5 text-sm font-black text-white shadow-md transition active:scale-[.98] disabled:opacity-40"><Download size={18}/>Импортировать и включить</button>
          <p className="mt-3 text-xs leading-relaxed text-slate-500 dark:text-slate-400">Сначала подписка разбирается и проверяется. TPROXY переключается только после успешного импорта — ошибка подписки больше не должна ронять сеть.</p>
        </section>

        <section className={`${card} p-5`}>
          <div className="flex items-center justify-between"><div><h3 className="font-bold">Маршрутизация</h3><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Happ routing подхватывается автоматически</p></div><Route className="text-indigo-500"/></div>
          <div className="mt-4 grid grid-cols-3 gap-1 rounded-2xl bg-slate-100 p-1 dark:bg-white/5">{([['routing','Правила'],['global','Весь трафик'],['direct','Напрямую']] as Array<[EasyMode,string]>).map(([m,label]) => <button key={m} onClick={() => void run(`mode:${m}`, 'Меняю режим', async () => { const next = await easyBridge.setMode(m); setState(next); if (status.running) await handleServiceAction('restart'); })} className={`rounded-xl px-1 py-2.5 text-[11px] font-bold transition ${state.mode === m ? 'bg-white text-indigo-700 shadow-sm dark:bg-slate-700 dark:text-indigo-300' : 'text-slate-500'}`}>{label}</button>)}</div>
          {state.mode === 'routing' && state.routings.length > 0 && <select value={state.activeRouting || ''} onChange={e => void run('routing', 'Применяю routing', async () => { const next = await easyBridge.setRouting(e.target.value); setState(next); if (status.running) await handleServiceAction('restart'); })} className="mt-3 w-full rounded-2xl bg-slate-100 px-4 py-3 text-sm dark:bg-white/5"><option value="">Базовые правила</option>{state.routings.map(r => <option key={r.id} value={r.id}>{r.name}{r.autoEnable ? ' · AUTO' : ''}</option>)}</select>}
        </section>

        <section className={`${card} divide-y divide-slate-100 p-2 dark:divide-white/5`}>
          <button onClick={() => setView('nodes')} className="flex w-full items-center gap-3 rounded-2xl p-3 text-left"><div className="rounded-2xl bg-emerald-100 p-3 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300"><Gauge size={20}/></div><div className="flex-1"><b className="text-sm">Узлы и задержка</b><p className="text-xs text-slate-500 dark:text-slate-400">{nodes.length ? `${nodes.length} доступно` : 'Пока пусто'}</p></div><ChevronRight size={18} className="text-slate-400"/></button>
          <button onClick={() => setView('settings')} className="flex w-full items-center gap-3 rounded-2xl p-3 text-left"><div className="rounded-2xl bg-amber-100 p-3 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300"><Settings2 size={20}/></div><div className="flex-1"><b className="text-sm">Core и сеть</b><p className="text-xs text-slate-500 dark:text-slate-400">{state.core} · {network.proxyMode === 2 ? 'REDIRECT' : network.proxyMode === 1 ? 'TPROXY' : 'AUTO TPROXY'}</p></div><ChevronRight size={18} className="text-slate-400"/></button>
        </section>
      </div>}

      {view === 'nodes' && <div className="space-y-4">
        <div className="flex items-center justify-between px-1"><div><h2 className="text-2xl font-black">Узлы</h2><p className="text-xs text-slate-500 dark:text-slate-400">Выбор работает независимо от Clash UI</p></div><button onClick={() => void testAll()} disabled={!nodes.length || busy === 'ping-all'} className="flex items-center gap-2 rounded-full bg-indigo-100 px-4 py-2 text-xs font-bold text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-300"><CircleGauge size={16}/>Проверить все</button></div>
        <div className="relative"><Search size={18} className="absolute left-4 top-3.5 text-slate-400"/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Страна, сервер, протокол…" className="w-full rounded-[22px] bg-white py-3 pl-11 pr-4 text-sm shadow-sm outline-none ring-indigo-500 focus:ring-2 dark:bg-[#1b1b20]"/></div>

        {filteredNodes.length === 0 ? <section className={`${card} p-8 text-center`}><Server className="mx-auto text-slate-400" size={36}/><h3 className="mt-3 font-bold">Узлов нет</h3><p className="mt-1 text-sm text-slate-500">Импортируй подписку на главной. Если JSON валиден, узлы появятся здесь ещё до запуска core.</p></section> : <div className="space-y-2">{filteredNodes.map(row => {
          const selected = state.selectedNode === row.ref;
          const l = latency[row.ref];
          return <section key={row.ref} className={`${card} flex items-center gap-3 p-3 ${selected ? 'ring-2 ring-indigo-500' : ''}`}>
            <button onClick={() => void selectNode(row.ref)} className="flex min-w-0 flex-1 items-center gap-3 text-left"><div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${selected ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 dark:bg-white/5 dark:text-slate-300'}`}>{selected ? <Check size={20}/> : <Globe2 size={20}/>}</div><div className="min-w-0"><b className="block truncate text-sm">{row.label}</b><span className="block truncate text-[11px] text-slate-500 dark:text-slate-400">{row.group} · {nodeProtocol(row.node)}</span></div></button>
            <button onClick={() => void testNode(row)} className={`min-w-[78px] rounded-2xl px-3 py-2 text-xs font-bold ${l && !('testing' in l) && l.ok ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300' : 'bg-slate-100 text-slate-600 dark:bg-white/5 dark:text-slate-300'}`}>{formatLatency(l)}</button>
          </section>;
        })}</div>}
      ...
      {view === 'settings' && <div className="space-y-4">
        <div className="px-1"><h2 className="text-2xl font-black">Core и сеть</h2><p className="text-xs text-slate-500 dark:text-slate-400">Без скрытых Clash-настроек</p></div>
        <section className={`${card} p-4`}><h3 className="px-1 pb-3 font-bold">Движок</h3><div className="grid grid-cols-2 gap-2">{CORE_INFO.map(c => <button key={c.value} disabled={Boolean(busy)} onClick={() => void switchCore(c.value)} className={`relative rounded-[22px] border p-4 text-left transition ${state.core === c.value ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-500/10' : 'border-slate-200 dark:border-white/10'}`}><div className="flex items-center justify-between"><Server size={19} className={state.core===c.value?'text-indigo-600':'text-slate-500'}/>{state.core===c.value&&<Check size={17} className="text-indigo-600"/>}</div><b className="mt-3 block text-sm">{c.title}</b><span className="mt-1 block text-[10px] leading-relaxed text-slate-500">{c.subtitle}</span>{c.recommended&&<span className="mt-2 inline-block rounded-full bg-indigo-100 px-2 py-1 text-[9px] font-bold text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-300">РЕКОМЕНДУЕТСЯ</span>}</button>)}</div></section>
    ...
    <nav className="fixed bottom-0 left-1/2 z-40 flex w-full max-w-md -translate-x-1/2 items-center justify-around border-t border-slate-200/70 bg-[#f1eff9]/95 px-3 pb-[max(env(safe-area-inset-bottom),8px)] pt-2 backdrop-blur-xl dark:border-white/10 dark:bg-[#1b1b20]/95">
      {([{id:'home',label:'Главная',icon:House},{id:'nodes',label:'Узлы',icon:Activity},{id:'settings',label:'Настройки',icon:Settings2}] as const).map(item => { const Icon=item.icon; const active=view===item.id; return <button key={item.id} onClick={()=>setView(item.id)} className={`flex min-w-[92px] flex-col items-center gap-1 rounded-2xl py-1.5 text-[10px] font-bold ${active?'text-indigo-700 dark:text-indigo-300':'text-slate-500'}`}><span className={`rounded-full px-5 py-1 ${active?'bg-indigo-200/70 dark:bg-indigo-500/20':''}`}><Icon size={20}/></span>{item.label}</button>; })}
    </nav>
  </div>;
}
